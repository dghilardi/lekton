...
- Guardrails against runaway AI usage: a dedicated rate limit on the endpoints that call an LLM, keyed by authenticated user instead of IP so one caller cannot consume a whole office's quota; a cap on how many generations a single caller may have in flight (`usage.max_concurrent_per_caller`, default 2); and an optional instance-wide daily spend ceiling (`usage.daily_credit_cap`, off by default) as a last line of defence against a runaway job.
- The daily AI spend ceiling now actually stops a runaway reindex. Only attachment transcription was admission-checked, and embeddings — the bulk of what indexing costs — went through uncounted against the limit: a live run spent 296 credits against a ceiling of 5 and finished undisturbed. Embedding calls are now refused once the caller's ceiling is reached; cache hits still pass, since they cost nothing.
- Attachment extraction now waits when the daily AI spend reserve for background work is used up, instead of failing. A failed extraction is marked `Failed`, and the startup sweep only resumes `Pending`/`InProgress` work — so hitting the ceiling used to lose the attachment permanently. It now resumes on its own once the reserve clears.
- A full RAG reindex now reports what it is costing, extrapolated from the items that actually indexed: the credits spent so far and a projection for the whole run, logged periodically and again on completion. The cost of a reindex is not knowable up front — it depends on how the corpus chunks — so the run measures itself rather than guessing, and the projection lands early enough to act on.
- Query embeddings are now attributed to the user who asked, instead of being lumped in with background indexing. Previously the usage report showed a single `system` figure covering both, which is the one distinction the report exists to make.
- Per-caller AI spending budgets (`usage.budget`, off by default): a credit bucket that refills over time, with named plans an administrator assigns per user. Plans are deliberately independent of access levels — what someone may read says nothing about how much they need to spend. A caller near the bottom of their budget is served without the optional retrieval steps (query rewriting, complexity analysis, HyDE, reranking) rather than refused, and a refusal says when the budget refills.
- The daily AI spend ceiling now stops background indexing at 80% of the limit, keeping the remainder for people. Previously a single shared ceiling got the priority backwards: an overrunning index spent the whole allowance, the refusals landed on users, and the index itself — which was never admission-checked — carried on regardless.
- Admin AI-usage report at `/admin/ai-usage`: who spent what over the last day, week or month, ranked by credits and priced per model. Background indexing appears as `system`, since it has no caller to bill. Reads the per-caller event log, and says so when it is off rather than showing an empty table.
- Per-model pricing (`[usage.pricing]`) that converts tokens into credits, so spend is comparable across models rather than being counted in tokens that differ in price by orders of magnitude. A model with no entry is charged at a flat fallback rate and counted in `lekton_llm_unpriced_calls_total`, so a gap in the price list is visible instead of free.
...
### Added
- Documentation release versioning (behind the `doc_versioning` feature flag, off by default): a source can publish named releases, so several versions of the same documentation stay readable. `lekton-sync --version 1.2.0 [--latest]` stages and finalizes a hash-verified release before optionally moving the `latest` alias, following the Docker tag model; incomplete releases cannot be selected or promoted, promotion enforces the token's scopes across every affected document and re-indexes only the slugs whose `latest` membership changed, and archiving is scoped to the exact release without deleting another release's search/RAG entry. A source's first release takes the `latest` alias on its own, and only a finalized first release archives the unversioned set it supersedes, so a publish that fails partway leaves readers where they were. Readers switch release from a selector on the document page (hidden when there is nothing to choose) and pinned sources are declared in a strip in the shell, with every pin preserved across navbar/sidebar links, breadcrumbs, search, citations, fragments, unrelated query parameters, and rendered Markdown; a pin naming a release that no longer exists degrades to `latest`, and re-syncing a published release leaves it selectable while the sync is in flight. Search and RAG reconcile one `latest` copy per slug, and alias moves persist a retryable per-slug backlog until background indexing or deletion succeeds. Asset references and attachment ACLs identify the exact `(slug, release)` pair. The editorial history is renamed from "version" to "revision" to keep it distinct from a release.
...
### Fixed
- `lekton-sync --dry-run` no longer archives documents on the server. Sync is called even under `--dry-run` and performs the archiving itself, so `--dry-run --archive-missing` was applying the archive it was only meant to preview.
