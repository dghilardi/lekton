...
### Added
- **Automatic token refresh with deduplication**: When an access token expires mid-session, the client now detects the 401 sentinel, calls `POST /auth/refresh` once (regardless of how many concurrent requests failed simultaneously), retries the original call, and redirects to `/login` only if the refresh token is also expired or revoked.
