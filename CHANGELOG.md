...
### Added
- Documentation release versioning (behind the `doc_versioning` feature flag, off by default): a source can publish named releases, so several versions of the same documentation stay readable. `lekton-sync --version 1.2.0 [--latest]` stages and finalizes a hash-verified release before optionally moving the `latest` alias, following the Docker tag model; incomplete releases cannot be selected or promoted, promotion enforces the token's scopes across every affected document, and archiving is scoped to the exact release without deleting another release's search/RAG entry. Readers switch release from a selector on the document page (hidden when there is nothing to choose) and pinned sources are declared in a strip in the shell, with every pin preserved across navbar/sidebar links, breadcrumbs, search, citations, fragments, and rendered Markdown; a pin naming a release that no longer exists degrades to `latest`. Search and RAG reconcile one `latest` copy per slug, and alias moves persist a retryable per-slug backlog until background indexing or deletion succeeds. Asset references and attachment ACLs identify the exact `(slug, release)` pair. The editorial history is renamed from "version" to "revision" to keep it distinct from a release.
