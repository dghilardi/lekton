...
### Added
- Documentation release versioning (behind the `doc_versioning` feature flag, off by default): a source can publish named releases, so several versions of the same documentation stay readable. `lekton-sync --version 1.2.0 [--latest]` publishes a release and optionally moves the `latest` alias, following the Docker tag model; archiving is scoped to the release being synced, so removing a file no longer takes it out of the releases that still ship it, and a source that has published a release must keep naming one. Readers switch release from a selector on the document page (hidden when there is nothing to choose) and pinned sources are declared in a strip in the shell, with pins travelling in the page URL so a pinned view is shareable and survives navigation; a pin naming a release that no longer exists degrades to `latest`. Search and RAG follow `latest` only, and the alias move re-indexes the affected documents in the background. The editorial history is renamed from "version" to "revision" to keep it distinct from a release.
