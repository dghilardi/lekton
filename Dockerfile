...
# Guarantee every served static asset is in target/site/js even when cargo
# considers the build up-to-date and skips the public/ -> target/site/ sync.
# Same script is used by the e2e workflow so the two environments never drift.
RUN bash scripts/stage-site-assets.sh
