...
pub fn AdminSidebar() -> impl IntoView {
    ...
    view! {
        <ul class="flex flex-col gap-1 mt-6">
            ...
            <Show when=move || upload_enabled.get()>
            <li>
            ...
            <li>
                <a href="/admin/ai-usage" aria-current=move || if pathname.get() == "/admin/ai-usage" { Some("page") } else { None } class="gap-3 group transition-colors">
                    <svg class="w-4 h-4 opacity-70" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 16v-5"/><path d="M12 16V8"/><path d="M17 16v-9"/></svg>
                    "AI Usage"
                </a>
            </li>
            <li>
